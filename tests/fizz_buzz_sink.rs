...
#[async_trait]
impl EventSink<N> for Filter {
    type Error = <Filter as EventSinkExt<N>>::Error;
    async fn on_event_ref(
        &self,
        event: &N,
        source: Option<Arc<EventBox>>,
    ) -> Result<(), Self::Error> {
        <Filter as EventSinkExt<N>>::on_event(&self, Cow::Borrowed(event), source).await
    }
    async fn on_event_owned(
        &self,
        event: N,
        ...
        <Filter as EventSinkExt<N>>::on_event(&self, Cow::Owned(event), source).await
    }
}
